...
def caption(report_date, model, skipped):
    ...
    return (
        ...
        f"Colour is the cell's own value, not the model's verdict; the small "
        f"triangle at the left edge is {model.title}'s full-setup verdict on its own "
        f"window, and a crowded row with no triangle is a market whose other legs "
        f"block the gate. A trailing window renormalises every week, so a short "
