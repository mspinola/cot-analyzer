...
def build_signal_panel(df, asset, color_palette, target_date=None, is_equity=False,
    ...
    try:
        ...
        card_positioning = _make_signal_card(
            ...
            # The short names label the legs in place; the bare "Index: 14, 85, 64"
            # said nothing a new reader could use, and there is no hover on the
            # phones this panel is read from. CFTC column prefixes, not C / L / S.
            html.Small([
                f"Index: {vc.LEG_SHORT[vc.LEG_COMM]} ",
                _colored_span(comm_idx, comm_idx >= max_idx, comm_idx <= min_idx),
                f" · {vc.LEG_SHORT[models.LEG_LARGE]} ",
                _colored_span(lrg_idx, lrg_idx >= max_idx, lrg_idx <= min_idx),
                f" · {vc.LEG_SHORT[models.LEG_SMALL]} ",
...
def index_triplet(row, model, size="0.85rem"):
    ...
    # Each value wears its leg's short name, tiny and dim: the triplet was the most
    ...
    # touch screen has no hover. The names are labels, not readings, so they stay
    # dim whether or not the leg is lit. They are the CFTC column prefixes
    # (vc.LEG_SHORT), which replaced the C / L / S letters of the CMR vocabulary.
    parts = []
    for i, (leg, value) in enumerate(values):
        if i:
            parts.append(html.Span("/", style={"color": vc.TEXT_COLOR, "opacity": 0.4,
                                               "margin": "0 2px"}))
        lit = gated[leg]
        parts.append(html.Span(vc.LEG_SHORT[leg],
